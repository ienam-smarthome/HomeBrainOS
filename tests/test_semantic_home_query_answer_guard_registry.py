...
def test_semantic_home_query_route_returns_none_for_write_passthrough():
    ...
    app = SimpleNamespace(semantic_home_evidence_broker=Broker(), ollama=SimpleNamespace())
...
def test_semantic_home_summary_runs_as_registry_terminal_route():
    ...
    class Client:
        ...
        async def post(self, url, **kwargs):
            self.calls += 1
            return Response('{"intent":"home_summary"}' if self.calls == 1 else "Everything looks calm at home.")
    class Broker:
        async def collect(self, limit):
            assert limit == 20
            return {"success": True, "data": {"lights": {"on_count": 0}, "contacts": {"open_count": 0}}, "tools_used": ["home_snapshot"]}
    app = SimpleNamespace(
        ask=None,
        semantic_home_evidence_broker=Broker(),
        ollama=SimpleNamespace(_http=Client(), planner_model="test-model", cloud_model="test-model", model="test-model", base_url="http://ollama", keep_alive="30m"),
    )
    async def base(request):
        return {"message": "base"}
    app.ask = base
    registry = AnswerGuardRegistry(app)
    registry.register_terminal_route("semantic-home-query", build_semantic_home_query_terminal_route(app))
...
def test_public_entrypoint_uses_semantic_home_query_in_shared_registry():
