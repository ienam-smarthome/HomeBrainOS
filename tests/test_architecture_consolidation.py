...
def test_current_release_metadata_is_aligned():
    ...
    assert 'version: "0.10.84"' in config
    assert 'PREVIOUS_RELEASE_VERSION = "0.10.83"' in entrypoint
    assert 'RELEASE_VERSION = "0.10.84"' in entrypoint
    assert "Current add-on version: **0.10.84**" in readme
