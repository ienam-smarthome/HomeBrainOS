...
def test_direct_thermostat_query_is_terminal_before_delegation(monkeypatch):
    ...
    async def fake_read(application):
        calls.append("read")
        return ({"label": "Thermostat", "temperature": 21.0, "heating_setpoint": 19.0, "cooling_setpoint": None}, {"tools_used": []})
    ...
    registry.register_terminal_route("thermostat-live-state", build_thermostat_terminal_route(app))
    registry.register_guard("thermostat-summary", build_thermostat_summary_guard(app))
...
def test_summary_guard_runs_after_delegation_and_preserves_evidence(monkeypatch):
    ...
    async def fake_read(application):
        calls.append("read")
        return ({"label": "Thermostat", "temperature": 25.0, "heating_setpoint": 20.0, "cooling_setpoint": None}, {"source": "test", "tools_used": []})
    async def base(request):
        calls.append("base")
        return {"message": "The thermostat is set to 25°C.", "display": {"summary": "The thermostat is set to 25°C."}}
    monkeypatch.setattr(registry_module, "_live_thermostat_reading", fake_read)
    app = SimpleNamespace(ask=base)
    registry = AnswerGuardRegistry(app)
    registry.register_terminal_route("thermostat-live-state", build_thermostat_terminal_route(app))
...
def test_public_entrypoint_preserves_final_thermostat_registry_position():
    source = ENTRYPOINT.read_text(encoding="utf-8")
    semantic_position = source.index('"semantic-home-registry"')
    thermostat_position = source.index('"summary-thermostat-registry"')
    read_position = source.index('"read-execution-registry"')
    assert semantic_position < thermostat_position < read_position
    assert 'summary_thermostat_registry.register_terminal_route(\n    "thermostat-live-state"' in source
    assert 'summary_thermostat_registry.register_guard(\n    "thermostat-summary"' in source
