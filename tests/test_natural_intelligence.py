...
class FakeMain:
    ...
    def energy_advisor_answer(self):
        return {'success': True, 'message': 'Your home is using 418 watts.'}
    ...
    def recommendations_answer(self):
        return {'success': True, 'items': ['Nothing urgent.']}

    def all_devices(self):
        return [
            {'label': 'TV', 'attributes': {'power': '86'}},
            {'label': 'Fridge', 'attributes': {'power': 89}},
...
def test_home_context_facade_uses_existing_app_functions():
    ...
    assert context['energy']['message'] == 'Your home is using 418 watts.'
    assert context['top_power_consumers'][0]['label'] == 'Fridge'
    assert context['top_power_consumers'][0]['power'] == '89 watts'


def test_register_adds_stable_v15_endpoint_aliases_once():
    module = load_natural_intelligence()
    fake = FakeMain()
    ...
    assert fake.APP_VERSION == '1.5.0-alpha'
    assert fake.app.version == '1.5.0-alpha'
