...
def test_named_rule_status_route_runs_as_registry_terminal_route():
    class Result:
        is_error = False
        data = {"rules": [{"id": 42, "name": "Night lights", "disabled": False, "paused": False}]}
    class Controller:
        class MCP:
            async def call_tool(self, name, arguments):
        ...
        mcp = MCP()
        @staticmethod
        def _rule_rows(data): return [{"id": 42, "name": "Night lights"}]
        @staticmethod
        def _exact_matches(rules, intent): return rules
        @staticmethod
        def _possible_matches(rules, intent): return []
    ...
    registry.register_terminal_route("named-rule-status", build_named_rule_status_terminal_route(Controller()))
...
def test_public_entrypoint_uses_named_rule_status_in_read_execution_registry():
