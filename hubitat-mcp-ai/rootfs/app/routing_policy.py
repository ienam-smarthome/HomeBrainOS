...
_FAST_READ_PATTERNS = (
    ...
    r"^(?:show\s+)?(?:the\s+)?(?:total\s+|combined\s+)?lights?\s+on\s+time(?:\s+(?:for\s+)?today)?\??$",
    r"^how\s+(?:long|much\s+time)\s+(?:have\s+)?(?:all\s+)?(?:the\s+)?lights?\s+(?:been\s+)?on\s+today\??$",
    r"^(?:show|calculate|get|give\s+me)\s+(?:the\s+)?(?:total\s+|combined\s+)?(?:daily\s+)?lights?\s+(?:on\s+time|usage)\s+(?:for\s+)?today\??$",
