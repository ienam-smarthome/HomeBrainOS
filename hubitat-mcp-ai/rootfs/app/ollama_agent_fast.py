...
class OllamaMCPAgent:
    """Bounded Ollama tool loop with correct tool-result association."""
    ...
    async def answer(
        ...
        return {
            ...
            "message": (
                "The request reached the local tool-step limit. Try a more specific "
                "question, or use one of the fast shortcuts."
            ),
            "model": self.model,
            "tools_used": tools_used,
    ...
    def _select_tools(self, query: str, tools: list[MCPTool]) -> list[MCPTool]:
        ...
        if re.search(r"\b(turn|switch|set|lock|unlock|open|close)\b", q):
            must_include.add("hub_call_device_command")
