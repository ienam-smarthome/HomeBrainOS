...
async function submit(q){q=(q||input.value).trim();if(!q)return;input.value='';bubble('user',q);history.push({role:'user',content:q});save();ask.disabled=true;ask.textContent='Working…';try{const r=await fetch('/api/ask',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:q,history:history.slice(-10)})});const a=await r.json();const text=a.message||a.detail||'No response';bubble('assistant',text,`Route: ${a.route||'unknown'}${a.model?' · '+a.model:''}`);history.push({role:'assistant',content:text});save();document.getElementById('lastRoute').textContent=a.route||'—';document.getElementById('route').textContent='Route: '+(a.route||'unknown');if(window.speechSynthesis&&text.length<900){const u=new SpeechSynthesisUtterance(text.replace(/[-•]\s/g,''));u.rate=1;window.speechSynthesis.cancel();window.speechSynthesis.speak(u)}}catch(e){bubble('assistant','Request failed: '+e.message)}finally{ask.disabled=false;ask.textContent='Ask';status()}}
