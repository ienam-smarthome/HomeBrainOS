...
.mic-fab{position:fixed;right:18px;bottom:18px;z-index:10;width:64px;height:64px;margin:0;border-radius:50%;font-size:27px;background:var(--green);box-shadow:0 10px 30px rgba(0,0,0,.5)}.mic-fab.listening{background:#dc2626}.copy-button{width:auto;display:block;margin:10px 0 0 auto;padding:7px 10px;font-size:12px}.copy-ok{background:var(--green)}.copy-fail{background:var(--red)}
...
async function copyAnswer(value,button){let ok=legacyCopy(value);if(!ok&&globalThis.isSecureContext&&navigator.clipboard?.writeText){try{await navigator.clipboard.writeText(value);ok=true}catch(error){ok=false}}button.textContent=ok?'Copied':'Copy blocked — select text';button.className=`copy-button ${ok?'copy-ok':'copy-fail'}`;setTimeout(()=>{button.textContent='Copy answer';button.className='secondary copy-button'},1800)}
function showAnswer(data){answer.replaceChildren();const meta=document.createElement('div');meta.className='answer-meta';[data.route,data.model,data.elapsed_ms===undefined?null:`${(data.elapsed_ms/1000).toFixed(1)}s`].filter(Boolean).forEach(value=>{const badge=document.createElement('span');badge.className='badge';badge.textContent=value;meta.appendChild(badge)});if(meta.children.length)answer.appendChild(meta);const text=document.createElement('div');text.className='answer-text';text.textContent=data.message||data.detail||'No response';answer.appendChild(text);if(/please confirm/i.test(text.textContent)){const confirm=document.createElement('button');confirm.className='confirm';confirm.textContent='Confirm action';confirm.onclick=()=>submit('confirm');answer.appendChild(confirm)}const copy=document.createElement('button');copy.className='secondary copy-button';copy.textContent='Copy answer';copy.onclick=()=>copyAnswer(text.textContent,copy);answer.appendChild(copy);const details=document.createElement('details');details.className='technical';const summary=document.createElement('summary');summary.textContent='Technical details';const pre=document.createElement('pre');pre.textContent=JSON.stringify(data,null,2);details.append(summary,pre);answer.appendChild(details);document.getElementById('lastRoute').textContent=data.route||'—';document.getElementById('lastTime').textContent=data.elapsed_ms===undefined?'—':`${(data.elapsed_ms/1000).toFixed(1)}s`;if(readAnswers&&speechSynthesis&&text.textContent.length<1500){speechSynthesis.cancel();speechSynthesis.speak(new SpeechSynthesisUtterance(text.textContent))}}
