...
class TruthfulHomeSnapshotService(HomeSnapshotService):
    ...
    async def answer(self, query: str) -> dict[str, Any]:
        ...
        if devices and snapshot.get("states_read", 0) == 0:
            ...
            recovered_devices, recovered_diagnostics, recovered_hub = (
        ...
        display = display_payload(
            ...
            metrics=self._truthful_metrics(
                snapshot,
                states_available=states_available,
            ),
...
SnapshotService = TypeVar("SnapshotService", bound=HomeSnapshotService)


def _install_snapshot(
    service_type: type[SnapshotService],
    application: Any,
    device_index: Any,
    *,
    ai_enabled: bool,
