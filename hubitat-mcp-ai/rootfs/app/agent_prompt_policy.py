...
def build_system_prompt(
    ...
    *,
    ...
    return (
        ...
        "uses start and end maps such as "
        "start={'type':'clock','time':'02:30'} and "
        ...
        "- ROUTINE DEVICE CONTROL: For routine light or switch state changes, call "
        "homebrain_control_devices once with exact room or device_names, device_kind, "
        "and semantic command arguments. Supported routine actions are on/off/toggle, "
        ...
        "when this adapter can express the intent. Call it only when the user "
        "explicitly requests a state change; only when the user explicitly requests a state change "
        "may this routine adapter mutate devices. device_names must contain ONLY actual "
        ...
        "call homebrain_control_devices for this -- its routine switch/brightness "
        "actions reject blockInternet/allowInternet outright, and turning a device's "
        ...
        "configuration or execution evidence when it explains downstream behavior; "
        "then mode/location or environmental correlation. If the needed rule/app/"
        "log gateway is not declared, use hub_search_tools for that evidence class "
        "instead of guessing a gateway. Stop when the user's causal question is "
        "answered or the bounded evidence budget is exhausted; do not sweep every "
        "device merely to be thorough. Never substitute current manifests for event "
        "history, and never turn timing correlation alone into proven causation.\n"
