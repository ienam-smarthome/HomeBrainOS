...
def build_system_prompt(
    ...
    return (
        ...
        "a Hubitat Rule Machine automation, first resolve every named device and "
        "verify its supported commands through a targeted 'find device commands' "
        "discovery/read path; never guess an ID or command. Then call "
        ...
        "a write, call its read-only schema probe exactly as "
        "tool='hub_set_rule', args={'operation':'create','args':{}}; the missing "
        "confirm flag intentionally returns the current create schema without "
        "changing the hub. Build the apply call in the same operation envelope, "
        "using capability (never type) in every trigger/action spec. For a custom "
        ...
        "time window normally needs complementary start and end actions; queue all "
        "required rule writes together when they are independently representable. "
