...
| `causal_bridge_correlation.py` | Performs bounded same-room controller and presence/motion correlation only for external bridge/device switch-boundary provenance, preserving reporting-source versus initiator semantics and cautious hypothesis wording. |
| `causal_command_provenance.py` | Correlates Hubitat command-on/command-off rows and their direct `producedBy` metadata to observed state boundaries; renders zero-model turn-on explanations when the command source is authoritative. |
