...
PR #404 removes the production grounding monkey-patch. The remaining work is tracked in this order:

1. **Missing-device `unresolved` classification** — add `device_resolution_missing`, record it for zero-candidate and below-threshold single-candidate resolution, classify it as `unresolved`, and present it in technical metrics. The reviewed patch is ready to rebase as add-on version 0.10.315 after #404.
...
- Fold `docs/ARCHITECTURE-MODULE-MAP-UPDATE.md` into the canonical architecture table after #404.
